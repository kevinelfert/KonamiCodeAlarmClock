...
class App():
    ...
    def __init__(self):
        ...
        # main label
        self.main_lbl = tk.Label(self.main_window, text = 'Konami Code Alarm Clock', font = ('calibri', 40, 'bold'))
        self.main_lbl.pack(anchor=tk.CENTER)

        # current time label
        self.time_lbl = tk.Label(self.main_window, text = 'Current Time', font = ('calibri', 30, 'bold'))
        self.time_lbl.pack(anchor=tk.CENTER)
        
        # time label
        self.current_time_lbl = tk.Label(self.main_window, font = ('calibri', 30, 'bold'))
        self.current_time_lbl.pack(anchor=tk.CENTER)
        self.time()

        # set alarm label 
        self.set_alarm_lbl = tk.Label(self.main_window, text = 'Alarm Time', font = ('calibri', 30, 'bold'))
        ...
        self.minutes = ('00','05', '10', '15', '20', '25', '30', '35',
            '40', '45', '50', '55', '60')
        ...
        self.ante_meridian = tk.Radiobutton(self.frame, text='AM', variable=self.ampm, value='AM', indicatoron=False, font = ('calibri', 10, 'bold'))
        self.ante_meridian.configure(height=3, width=5)

        self.post_meridian = tk.Radiobutton(self.frame, text='PM', variable=self.ampm, value='PM', indicatoron=False, font = ('calibri', 10, 'bold'))
        ...
        self.set_alarm_btn = tk.Button(self.main_window, text='Set Alarm', command=lambda: self.set_alarm(self.hour.get(), self.minute.get(), self.ampm.get()))
    ...
    def time(self):
        ...
        if(self.current_time_string == '07:56:00 PM'):
            self.start_alarm()


    def set_alarm(self, hour, minute, ampm):
        alarm_time_string = '{hr}:{min}:00 {mer}'.format(hr=hour, min=minute, mer=ampm)
        print(alarm_time_string)
